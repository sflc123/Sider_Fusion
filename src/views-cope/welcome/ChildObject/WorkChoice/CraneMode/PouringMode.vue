...
const ExecuteWork2 = () => {
  ...
  axios
    .get(props.uRL[4], {
      params: {
        loadLeft: sendputlocation.value.loadLeft,
        loadRight: sendputlocation.value.loadRight,
        unloadLeft: sendputlocation.value.unloadLeft,
        unloadRight: sendputlocation.value.unloadRight
      }
    })
    .then(res => {
      console.log(res.data);
...
const op1 = () => {
  axios
    .get(props.ApiUrl.DataBase.GetGroundDivision)
    .then(res => {
...
const pourNo = ref();
const SelectPourNo = () => {
  axios.get(props.ApiUrl.DataBase.GetPourPoints).then(res => {
    pourNo.value = res.data;
  });
};
onMounted(async () => {
  await op1();
    ...
    <el-row>
      <el-col>
        <el-button
          ...
          >执行</el-button
        >
        <el-button class="w-2/12" size="large" type="warning" @click="PauseWork"
          >暂停</el-button
        >
        <el-button
          class="w-2/12"
          ...
          >继续</el-button
        >
        <el-button class="w-2/12" size="large" type="primary" @click="EndWork"
          >结束</el-button
        >
      </el-col>
    </el-row>

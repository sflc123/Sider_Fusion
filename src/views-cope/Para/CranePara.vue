...
const CarsParmsSetUp = {
  //大小车参数设置API
  SetLargeVehicle: Urls.PlcSetUp.SetLargeVehicle,
  SetSmallVehicle: Urls.PlcSetUp.SetSmallVehicle,
  //抓斗起升开闭参数设置API
  SetHoist: Urls.PlcSetUp.SetHoist,
  SetSwitch: Urls.PlcSetUp.SetSwitch,
  SetBucket: Urls.PlcSetUp.SetBucket,
  //限位系统参数设置API
  SetElectricLimit: Urls.PlcSetUp.SetElectricLimit,
  SetSystem: Urls.PlcSetUp.SetSystem
};
const CarParams = [
...
const GetLimitData = async () => {
  try {
    ...
    console.log(LimitParamsData.value);
  } catch (error) {
    console.error("Error fetching data:", error);
  }
};
...
<template>
  <el-tabs tab-position="top" type="border-card" style="margin: 0rem">
    <el-tab-pane label="大小车参数设置">
      <el-row>
        <el-col v-for="(param, index) in CarParams" :key="index" :span="10">
    ...
    <el-tab-pane label="抓斗参数设置">
      <el-row>
        <el-col v-for="(param, index) in BucketParams" :key="index" :span="7">
    ...
    <el-tab-pane label="限位参数设置">
      <el-row>
        <el-col v-for="(param, index) in LimitParams" :key="index" :span="10">
          <el-card shadow="always" style="height: 100%" class="pl-10">

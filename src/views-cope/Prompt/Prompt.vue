...
const filterTableData = computed(() => {
  return temps.value.filter(data => {
    ...
    const matchesDateRange =
      ...
      (new Date(data.time) >= selectedDateRange.value[0] &&
        new Date(data.time) <= selectedDateRange.value[1]);
...
onMounted(async () => {
  try {
    const response = await axios.get(Urls.DataBase.GetPromptAmount);
    totalItems.value = Math.ceil(response.data.length / pageSize.value);
    fetchPageData();
  } catch (error) {
    console.error(error);
  }

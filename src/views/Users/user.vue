...
function onSubmit() {
  ...
  API.DataBase.AddOperator(requestUrl, sizeForm)
    .then(res => {
      if (res.status) ElMessage.success(res.message);
...
const fetchUsers = async () => {
  API.DataBase.GetAllOperator(requestUrl)
    .then(res => {
      res.forEach(item => {});
...
const updateRow = (index?: number, row?: User) => {
  ...
  API.DataBase.UpdateOperator(requestUrl, row)
    .then(res => {
